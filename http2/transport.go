...
type Transport struct {
	...
	Settings          []Setting
	OverrideSettings  map[SettingID]uint32
...
func (t *Transport) newClientConn(c net.Conn, addr string, singleUse bool) (*ClientConn, error) {
	...
	if t.OverrideSettings == nil {
		...
		for _, orderedKey := range http2HeaderOrder {
			for settingId, settingVal := range t.OverrideSettings {
				if settingName[settingId] == orderedKey {
					initialSettings = append(initialSettings, Setting{ID: settingId, Val: settingVal})
				}
			}
		}
	}
